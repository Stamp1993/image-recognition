...
struct neuralNetwork {
		...
		for (int l = 0; l < depth; l++) {
			...
			for (int j = 0; j < neurSize; j++) {
				...
				for (int i = 0; i < weightsSize; i++) {
					...
					double update = activations[l][i] * err[l + 1][j];
	...
	void batchBackpropagation(MatrixXd inputsVec, MatrixXd expectedresults, double acceptableErr) {
		...
		while (1) {
			double k;
			while ((k = learn(inputsVec, expectedresults))>acceptableErr) {
			...
			for (int l = 0; l < depth; l++) {
				...
				for (int j = 0; j < neurSize; j++) {
					...
					for (int i = 0; i < weightsSize; i++) {
						...
						double update = upd[i] / PRMatr.cols();



						(*layers[l].neurons)[j].weights[i] -= update;
